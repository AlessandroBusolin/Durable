        ...
        public void ConflictResolution_MergeChanges()
        {
            ...
            try
            {
                ...
                using (SqliteRepository<Author> repo = new SqliteRepository<Author>(connectionString, null, null, resolver))
                {
                    ...
                    // Name: original==incoming, current!=original â†’ keep current ("Updated Name") 
                    // CompanyId: original==incoming, current!=original â†’ keep incoming (but db wins due to current implementation)
